...
Telemetry for HUD + KLV comes from the same MQTT topics as the rest of SkyScan: `LOGGER_TOPIC` (camera-pointing / `rho_c`, `tau_c`, `zoom`, `distance`) and `OBJECT_TOPIC` (skyscan-c2 **Selected Object**: tail/`flight`, ICAO from `object_id`, lat/lon/alt, `track`, velocities, `relative_distance` / `distance_3d`, `squawk`, `object_type` / `cot_event_type`). Parses both `data_payload_type` + `data_payload` and top-level `"Selected Object"` JSON envelopes.
