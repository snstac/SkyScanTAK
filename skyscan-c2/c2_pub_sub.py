                ...
                if not object_ledger_df.empty: 
                    ...
                    if self.override_object:
                        ...
                        if not target_ledger_df.empty:
                            ...
                            target_ledger_df = target_ledger_df.copy()
                            target_ledger_df.loc[:, "selectable"] = True
                            if "skyscan_priority" not in target_ledger_df.columns:
                                target_ledger_df["skyscan_priority"] = 0
                            else:
                                target_ledger_df["skyscan_priority"] = pd.to_numeric(
                                    target_ledger_df["skyscan_priority"],
                                    errors="coerce",
                                ).fillna(0)
                            potential_target = target_ledger_df.sort_values(
                                by=["skyscan_priority", "relative_distance"],
                            ...
                            if self.tracked_object is not None:
                                ...
                                if not current_target_ledger.empty:
                                    ...
                                    if potential_target is not None:
                                        pot_pri = float(
                                            potential_target.get("skyscan_priority", 0)
                                        )
                                        cur_pri = float(
                                            current_target.get("skyscan_priority", 0)
                                        )
                                        if pot_pri > cur_pri:
                                            logging.info(
                                                "Switching target: higher skyscan_priority "
                                                f"({cur_pri} -> {pot_pri})"
                                            )
